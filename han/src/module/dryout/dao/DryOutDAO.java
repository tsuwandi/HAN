...
public class DryOutDAO {
	...
	private String updateDailyClosingQuery = "update dry_out set confirm_date=?, status=?, "
			+ "edit_date=?, edited_by=? where dry_out_code=? ";
	...
	public List<DryOut> getAll() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				dryOut.setTotalVolume(rs.getBigDecimal("total_volume"));
	...
	public void save(DryOut dryOut) throws SQLException {
		try {
			...
			insertStatement.setBigDecimal(4, dryOut.getTotalVolume());
	...
	public void update(DryOut dryOut) throws SQLException {
		try {
			...
			updateStatement.setBigDecimal(3, dryOut.getTotalVolume());
	...
	public void updateDailyClosing(DryOut dryOut) throws SQLException {
		try {
			...
			updateDailyClosingStatement.setString(2, dryOut.getStatus());
			updateDailyClosingStatement.setDate(3, DateUtil.getCurrentDate());
			updateDailyClosingStatement.setString(4, "timotius");
			updateDailyClosingStatement.setString(5, dryOut.getDryOutCode());
