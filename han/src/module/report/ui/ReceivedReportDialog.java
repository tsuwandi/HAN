...
public class ReceivedReportDialog extends JDialog{
private static final long serialVersionUID = 1L;
	
	private static final Logger LOGGER = Logger.getLogger(ReceivedReportDialog.class);
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(doValidate(dryStockFlow) == true) {
					...
					List<ReceivedReport> results = new ArrayList<ReceivedReport>();
					...
					try {
						...
						results = ServiceFactory.getReportBL().getReceivedReport(sb.toString(),objs);
						...
						try {
							...
							try {
								JasperPrint jasperPrint = JasperFillManager.fillReport(
										"src/module/report/jasper/ReceivedReport.jasper", parameters, new JREmptyDataSource());
	...
	private boolean doValidate(DryStockFlow dryStockFlow) {
		...
		if(dryStockFlow.getStartFrom() == null || dryStockFlow.getEndTo() == null) {
			JOptionPane.showMessageDialog(null, "Periode Tidak Boleh Kosong.",
					"Laporan Detail Penerimaan Bahan Baku", JOptionPane.ERROR_MESSAGE);
			isValid = false;
		} else {
			isValid = true;
		}
