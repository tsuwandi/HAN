...
public class CustomerCreatePanel extends JPanel implements Bridging {
	...
	private CustomerCreatePanel supplierCreate;
	...
	protected void doSave() {
		...
		try {
			ServiceFactory.getCustomerBL().save(supplier, listOfSuppAddress);
	 ...
	 */
	protected void showAddSuppAddressDialog(CustomerCreatePanel supplierCreate) {
		CustomerAddressDialog suppAddressDialog = new CustomerAddressDialog(false, new SuppAddress(), supplierCreate, null);
		suppAddressDialog.setTitle("Alamat");
		suppAddressDialog.setLocationRelativeTo(null);
		suppAddressDialog.setVisible(true);
	}

	protected void showEditSuppAddressDialog(SuppAddress suppAddress, CustomerCreatePanel supplierCreate,
			Integer index) {
		CustomerAddressDialog suppAddressDialog = new CustomerAddressDialog(true, suppAddress, supplierCreate, index);
		suppAddressDialog.setTitle("Alamat");
		suppAddressDialog.setLocationRelativeTo(null);
		suppAddressDialog.setVisible(true);
	}
...
//			}
	...
	public String makeCodeNumber(int suppTypeId) {
		String constantSuppType = "";

		if (AppConstants.SUPP_TYPE_BALKEN_ID == suppTypeId) {
			constantSuppType = AppConstants.SUPP_TYPE_BALKEN_CD;
		} else if (AppConstants.SUPP_TYPE_BARECORE_ID == suppTypeId) {
			constantSuppType = AppConstants.SUPP_TYPE_BARECORE_CD;
		} else if (AppConstants.SUPP_TYPE_VENEER_ID == suppTypeId) {
			constantSuppType = AppConstants.SUPP_TYPE_VENEER_CD;
		} else if (AppConstants.SUPP_TYPE_FACEBACK_ID == suppTypeId) {
			constantSuppType = AppConstants.SUPP_TYPE_FACEBACK_CD;
		...
		if(!"".equals(constantSuppType)) {
			String ordinal = null;
			try {
				ordinal = ServiceFactory.getSupplierBL().getOrdinalOfCodeNumber(constantSuppType);
				
				return new StringBuilder().append(constantSuppType).append(ordinal).toString();
			} catch (SQLException e) {
				e.printStackTrace();
				LOGGER.error(e.getMessage());
				DialogBox.showErrorException();
