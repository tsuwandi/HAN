...
public class PopUpProdPKResult extends JDialog{
	Logger log = LogManager.getLogger(PopUpProdPKResult.class.getName());
	...
	private CreateProductionPKPanel createProductionPKPanel;
	private List<ProdPKResult> listOfPrd;
	...
	public PopUpProdPKResult(JPanel parent){
	...
	private void initData(JPanel parent){
		...
		if(createProductionPKPanel.getProduction().getListProdPKResult()!=null){
			listOfPrd = createProductionPKPanel.getProduction().getListProdPKResult();
			productionResultTable.setModel(new ResultTableModel(listOfPrd));
			for (ProdPKResult prd : listOfPrd) {
	...
	private void createGUI(){
		...
		//Table Area
		productionResultTableModel = new ResultTableModel(new ArrayList<ProdPKResult>());
		...
		productionResultTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(productionResultTable.columnAtPoint(e.getPoint())==6){
					...
					ProdPKResult prd = listOfPrd.get(productionResultTable.getSelectedRow());
					...
					for (ProdPKResultProduct pkpr : prd.getListProductPKResultProduct()) {
	...
	private void validatingDetail(){
		...
		if(error==0){
			if(editMode){
				ProdPKResult prd = listOfPrd.get(indexEdit);
				...
				if(prd.getListProductPKResultProduct().size()>0){
					for (ProdPKResultProduct prodPKResultProduct : prd.getListProductPKResultProduct()) {
				...
				ProdPKResult prd = new ProdPKResult();
				prd.setPressedNo(Integer.parseInt(pressNoField.getText()));
				if(!hourField.getText().equals("")||!minuteField.getText().equals("")){
					prd.setStartTime(hourField.getText()+":"+minuteField.getText());
				}else{
				...
				List<ProdPKResultProduct> prodPKResultProducts = new ArrayList<>();
				for(int i=0;i<6;i++){
					ProdPKResultProduct prodPK = new ProdPKResultProduct();
					if(i==0){
				...
				prd.setListProductPKResultProduct(prodPKResultProducts);
				listOfPrd.add(prd);
				pressMap.put(prd.getPressedNo(),prd.getPressedNo());
				productionResultTable.setModel(new ResultTableModel(listOfPrd));
				productionResultTable.updateUI();
	...
	private class ResultTableModel extends AbstractTableModel{
		private static final long serialVersionUID = 1L;
		private List<ProdPKResult> productionResults;
		    
		    public ResultTableModel(List<ProdPKResult> productionResults) {
		    ...
		    public Object getValueAt(int rowIndex, int columnIndex) {
		    	ProdPKResult p = productionResults.get(rowIndex);
		        switch(columnIndex){
		        	case 0:
