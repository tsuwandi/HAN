...
public class PopUpBigSearchMaterial extends JDialog{
	Logger log = LogManager.getLogger(PopUpBigSearchMaterial.class.getName());
	...
	List<ProdRM> prodRms;
	PopUpBigInputMaterial popUpInputMaterial;
	
	public PopUpBigSearchMaterial(JDialog parent){
		super(parent);
		createGUI(parent);
	}
	private void createGUI(JDialog parent){
		setLayout(null);
		setSize(700,400);
		popUpInputMaterial = (PopUpBigInputMaterial) parent;
		...
		try {
			prodRms = ServiceFactory.getProductionBL().getSearchProdRM(popUpInputMaterial.getProdRms(),popUpInputMaterial.getDeletedProdRms(),popUpInputMaterial.getProductionCode());
