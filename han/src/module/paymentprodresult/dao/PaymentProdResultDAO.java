...
public class PaymentProdResultDAO {
	...
	private String getAllQuery = new StringBuilder()
			.append("select id, pay_pr_code, rpr_code, due_date, rate, payment_status, subtotal, ")
			.append("disc, tax, other_fee, total, status, receive_date, supp_name, ppr_code, purchase_date, source, currency_abbr from ( ")
			.append("select pay_pr.id, pay_pr.pay_pr_code, pay_pr.rpr_code, pay_pr.due_date, pay_pr.rate, pay_pr.payment_status, pay_pr.subtotal, ")
			.append("pay_pr.disc, pay_pr.tax, pay_pr.other_fee, pay_pr.total, pay_pr.status, rpr.receive_date, s.supp_name, ppr.ppr_code, ppr.purchase_date, 'PAYMENT' as source, c.currency_abbr ")
			...
			.append("union ")
			.append("select r.id, null as pay_pr_code, r.rpr_code, null as due_date, null as rate, 'Belum' as payment_status, null as subtotal, ")
	...
	private String getAllRPRQuery = new StringBuilder()
			...
			.append("inner join supplier s on s.supp_code = ppr.supp_code ")
			.append("left join currency c on c.id = s.currency_id ")
			.append("where s.deleted_date is null and ppr.deleted_date is null and r.deleted_date is null and r.rpr_code not in (select rpr_code from payment_prod_result) ")
			.toString();
	...
	private String insertQuery = new StringBuilder()
			.append("insert into payment_prod_result (pay_pr_code, rpr_code, due_date, rate, payment_status, ")
	...
	private String updateQuery = new StringBuilder()
			...
			.append("edit_date=?, edited_by=? where pay_pr_code=?").toString();

	private String deleteQuery = "update payment_prod_result set deleted_date=?, deleted_by=? where id=?";

	private String getOrdinalOfCodeNumberQuery = "SELECT CONVERT(SUBSTRING_INDEX(rpr_code, '/', 1),UNSIGNED INTEGER) AS ordinal FROM payment_prod_result "
	...
	public void update(PaymentProdResult rpr) throws SQLException {
		try {
			...
			updateStatement.setString(13, rpr.getPayPrCode());
