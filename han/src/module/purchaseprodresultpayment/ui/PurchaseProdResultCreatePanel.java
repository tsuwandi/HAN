...
public class PurchaseProdResultCreatePanel extends JPanel implements Bridging {
	...
	public PurchaseProdResultCreatePanel() {
		...
		try {
			listOfSupplier = ServiceFactory.getPurchaseProductResultPaymentBL().getAllSupplierBySuppTypeId(3);
		...
		try {
			listOfCurrency = ServiceFactory.getPurchaseProductResultPaymentBL().getAllCurrency();
	...
	protected void doSave() {
		...
		try {
			ServiceFactory.getPurchaseProductResultPaymentBL().save(purchaseProductResult, listOfPPRProduct);
	...
	protected boolean doValidate() {
		...
		if (txtPurchaseProductResultCode.getText() == null || txtPurchaseProductResultCode.getText().length() == 0) {
			...
			try {
				if (ServiceFactory.getPurchaseProductResultPaymentBL().isPPRCodeExists(txtPurchaseProductResultCode.getText()) > 0) {
	...
	public void makeCodeNumber(Date producationDate) {
		...
		String ordinal = null;
		try {
			ordinal = ServiceFactory.getPurchaseProductResultPaymentBL().getOrdinalOfCodeNumber(Integer.valueOf(year));
		} catch (SQLException e) {
			LOGGER.error(e.getMessage());
			DialogBox.showErrorException();
		}
