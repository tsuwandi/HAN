...
public class ProductionWasteListPanel extends JPanel {
	...
	public ProductionWasteListPanel() {
		...
		lblBreadcrumb = new JLabel("ERP > Produksi > Input Hasil Produksi > Sisa Produksi");
		lblBreadcrumb.setFont(new Font("Tahoma", Font.BOLD, 12));
		lblBreadcrumb.setBounds(50, 10, 320, 30);
		add(lblBreadcrumb);

		lblHeader = new JLabel("Input Hasil Produksi Sisa Produksi ");
		...
		try {
			...
			listOfProductionWaste = ServiceFactory.getProductionWasteBL().getAllProductionWaste();
	...
	public void doSearch(String value) {
		try {
			listOfProductionWaste = new ArrayList<ProductionWaste>();
			listOfProductionWaste = ServiceFactory.getProductionWasteBL().getAllProductionWasteBySimpleSearch(value);
			refreshTableProductionWaste();
		} catch (SQLException e1) {
			LOGGER.error(e1.getMessage());
			DialogBox.showErrorException();
